...
var require_matcher_stream = __commonJS({
  "../node_modules/.pnpm/unzip-stream@0.3.4/node_modules/unzip-stream/lib/matcher-stream.js"(exports2, module2) {
    "use strict";
...
var require_entry = __commonJS({
  "../node_modules/.pnpm/unzip-stream@0.3.4/node_modules/unzip-stream/lib/entry.js"(exports2, module2) {
    "use strict";
...
var require_unzip_stream = __commonJS({
  "../node_modules/.pnpm/unzip-stream@0.3.4/node_modules/unzip-stream/lib/unzip-stream.js"(exports2, module2) {
    "use strict";
...
var require_parser_stream = __commonJS({
  "../node_modules/.pnpm/unzip-stream@0.3.4/node_modules/unzip-stream/lib/parser-stream.js"(exports2, module2) {
    "use strict";
    var Transform = require("stream").Transform;
    var util = require("util");
...
var require_unzip = __commonJS({
  "../node_modules/.pnpm/unzip-stream@0.3.4/node_modules/unzip-stream/unzip.js"(exports2) {
    "use strict";
...
async function getFilesIncludeName(directory, name) {
  const files = await import_fs_extra.default.readdir(directory, { withFileTypes: true });
  const filteredFile = files.find((item) => item.isDirectory() && item.name.includes(name));
  if (!filteredFile) {
    return "";
  }
  return import_path.default.join(directory, filteredFile.name);
}
...
function unzipFile(zipFilePath, outputFolderPath) {
  return new Promise((resolve2, reject) => {
    const readStream = import_node_fs.default.createReadStream(zipFilePath);
    import_fs_extra.default.ensureDirSync(outputFolderPath);
...
function downloadFile({
  ...
  const cleanup = () => {
    ...
    if (import_node_fs.default.existsSync(savePath)) {
      try {
        import_node_fs.default.unlinkSync(savePath);
      } catch (error) {
        console.log("\u6E05\u7406\u6700\u7EC8\u6587\u4EF6\u5931\u8D25:", error);
      }
    }
  };
  const cancel = () => {
  ...
  */
    ...
    if (fileInfo == null ? void 0 : fileInfo.length) {
      ...
      if (onlinePlugin.length) {
        for (const p of onlinePlugin) {
          ...
          if (existingPluginDir) {
            const version = extractVersionFromName(existingPluginDir);
            console.log(version, p.version, (0, import_compare_versions.compareVersions)(version, p.version));
            if ((0, import_compare_versions.compareVersions)(version, onlineVersion) >= 0) {
  ...
  async installFromPemox(pemoxPath, options) {
    ...
    try {
      ...
      try {
        ...
        await fs2.writeJSON(
          path2.join(this.config.pluginDir, "plugins.json"),
          pluginsConfigObject,
          { spaces: 2 }
        );
        if (isUpdate) {
  ...
  async installFromOnline(pluginManifest, options) {
    return new Promise(async (resolve2) => {
      try {
        ...
        await fs2.ensureDir(pluginPath);
        let downloadUrl = pluginManifest.link;
        if (process.platform === "darwin" && pluginManifest.macLink) {
          downloadUrl = pluginManifest.macLink;
        } else if (process.platform === "win32" && pluginManifest.winLink) {
          downloadUrl = pluginManifest.winLink;
        }
        if (!downloadUrl) {
          throw new PluginError(
            pluginId,
  ...
  cancelDownload(pluginId) {
    ...
    if (abortController) {
      ...
      this.downloadAbortControllers.delete(pluginId);
      this.cleanupDownloadFiles(pluginId);
    }
    return !!abortController;
  }
  /**
   * 清理下载的缓存文件
   * @param pluginId 可选的插件ID，如果不提供则清理所有下载文件
   */
  async cleanupDownloadFiles(pluginId) {
    try {
      if (pluginId) {
        await this.cleanupPluginDownloadFiles(pluginId);
      } else {
        await this.cleanupAllDownloadFiles();
      }
    } catch (error) {
   ...
   * @param pluginId 插件ID
   */
  async cleanupPluginDownloadFiles(pluginId) {
    try {
      const files = await fs2.readdir(this.config.pluginDir);
      const downloadFiles = files.filter(
        (file) => file.startsWith(`_download_${pluginId}_`) || file.startsWith(`_download_${pluginId}.downloading`)
      );
      for (const file of downloadFiles) {
        const filePath = path2.resolve(this.config.pluginDir, file);
        await fs2.remove(filePath);
        console.log(`\u5DF2\u6E05\u7406\u63D2\u4EF6 ${pluginId} \u7684\u4E0B\u8F7D\u6587\u4EF6:`, filePath);
      }
    } catch (error) {
      console.error(`\u6E05\u7406\u63D2\u4EF6 ${pluginId} \u7684\u4E0B\u8F7D\u6587\u4EF6\u5931\u8D25:`, error);
    }
  }
  /**
   * 清理所有下载文件
   */
  async cleanupAllDownloadFiles() {
    try {
      const files = await fs2.readdir(this.config.pluginDir);
      const downloadFiles = files.filter(
        (file) => file.startsWith("_download_") || file.startsWith("_download.downloading")
      );
      for (const file of downloadFiles) {
        const filePath = path2.resolve(this.config.pluginDir, file);
        await fs2.remove(filePath);
        console.log("\u5DF2\u6E05\u7406\u4E0B\u8F7D\u6587\u4EF6:", filePath);
      }
    } catch (error) {
      console.error("\u6E05\u7406\u6240\u6709\u4E0B\u8F7D\u6587\u4EF6\u5931\u8D25:", error);
    }
  }
  /**
   * 取消所有正在进行的下载
